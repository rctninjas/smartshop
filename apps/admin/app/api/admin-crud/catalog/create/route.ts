...
export async function POST(request: Request) {
  ...
  const response = await upstreamJson(request, '/api/catalog', {
    method: 'POST',
    body: {
    ...
    return NextResponse.json(
      {
        ...
        code: errorPayload.code ?? 'ADMIN_CATALOG_CREATE_FAILED',
        message: errorPayload.message ?? 'Failed to create product'
      },
      { status: response.status }
    );
  }
