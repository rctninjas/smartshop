...
- Задачи:
  ...
  - [ ] Модуль `customers`: API + UI + история заказов (выполнен базовый список и карточка; список включает зарегистрированных клиентов без заказов; доработка in_progress).
...
- `done`: Список клиентов в админке строится от зарегистрированных аккаунтов; клиенты отображаются даже без заказов, агрегаты заказов подтягиваются отдельно.
