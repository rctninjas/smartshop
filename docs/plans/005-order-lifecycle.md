...
- `done`: API `orders` реализован с list/get/create/update-status.
- `done`: правила переходов и ошибки `ORDER_INVALID_STATUS_TRANSITION`/`ORDER_TRACK_REQUIRED`.
- `done`: автоархив неоплаченных `created` заказов каждые 1 минуту при timeout 20 минут.
